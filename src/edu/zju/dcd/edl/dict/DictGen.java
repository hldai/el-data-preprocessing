...
//		public int pos = 0;
	...
	private static class NameStringCnt implements Comparable<NameStringCnt> {
		public ByteArrayString nameString = null;
		public int cnt = 0;
		
		@Override
		public int compareTo(NameStringCnt nsc) {
			return nameString.compareTo(nsc.nameString);
		}
	}
	
	public static void genNameStringCntFile(String midEachAliasCntFileName, String dstFileName) {
		...
		try {
//			LinkedList<NameStringCnt> nameStringCnts = new LinkedList<NameStringCnt>();
			
			String line = null;
			String preName = null;
			int cnt = 0;
			int curNameStringCnt = 0;
			while ((line = reader.readLine()) != null) {
				String[] vals = line.split("\t");
				
				if (preName != null && !preName.equals(vals[1])) {
//					NameStringCnt nsc = new NameStringCnt();
//					nsc.nameString = new ByteArrayString(preName);
//					nsc.cnt = curNameStringCnt;
//					nameStringCnts.add(nsc);
					
//					ByteArrayString bas = new ByteArrayString(preName);
//					bas.toFileWithShortByteLen(dos);
//					dos.writeInt(curNameStringCnt);
					writer.write(preName + "\t" + curNameStringCnt + "\n");
					curNameStringCnt = 0;
				}
				
				curNameStringCnt += Integer.valueOf(vals[2]);
...
//			Collections.sort(nameStringCnts);
//			
//			DataOutputStream dos = IOUtils.getBufferedDataOutputStream(dstFileName);
//			System.out.println(nameStringCnts.size());
//			dos.writeInt(nameStringCnts.size());
//			for (NameStringCnt nameStringCnt : nameStringCnts) {
//				nameStringCnt.nameString.toFileWithShortByteLen(dos);
//				dos.writeInt(nameStringCnt.cnt);
//			}
//			dos.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static void genSimpleWikiDict(String midEachAliasCntFileName, String midToWidFileName,
		...
		WidMidMapper mapper = new WidMidMapper(midToWidFileName);
...
//				dos.writeInt(entry.len);
	...
	public static void genDictWithPse(String midEachAliasCntFileName, String midAliasCntFileName,
			String nameStringCntFileName, String dstAliasFile,
		...
		BufferedReader reader = IOUtils.getUTF8BufReader(midEachAliasCntFileName);
		BufferedReader nameStringCntReader = IOUtils.getUTF8BufReader(nameStringCntFileName);
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (preName == null || !name.equals(preName)) {
					String[] vals = getNameStringCntVals(nameStringCntReader, name);
					if (vals == null)
						return ;
					
					nameStringCnt = Integer.valueOf(vals[1]);
...
//					len = 0;
				...
				midPseEntry.npse = (float)eachAliasCnt / nameStringCnt;
	...
	private static String[] getNameStringCntVals(BufferedReader reader, String name) throws IOException {
		String nameStringCntLine = reader.readLine();
		String[] vals = nameStringCntLine.split("\t");
		while (vals[0].compareTo(name) < 0) {
			nameStringCntLine = reader.readLine();
			vals = nameStringCntLine.split("\t");
		}
