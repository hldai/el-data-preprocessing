...
import edu.zju.dcd.edl.dict.DictGen;
import edu.zju.dcd.edl.io.IOUtils;
import edu.zju.dcd.edl.obj.ByteArrayString;
import edu.zju.dcd.edl.tac.DictStat;
...
public class GenDictMain {
	...
	public static void genDictPse(IniFile config) {
		...
		String midEachAliasCntFileName = sect.getValue("each_alias_cnt_file"), midAliasCntFileName = sect
				.getValue("alias_cnt_file"), nameStringCntFileName = sect.getValue("name_string_cnt_file"),
				dstAliasFile = sect.getValue("dst_alias_file"), dstMidFile = sect
				.getValue("dst_mid_file"), dstAliasIndexFile = sect
				.getValue("dst_alias_index_file");
		int numIndices = sect.getIntValue("num_indices");
		DictGen.genDictWithPse(midEachAliasCntFileName, midAliasCntFileName, nameStringCntFileName,
	...
	public static void genNameStringCntFile(IniFile config) {
		IniFile.Section sect = config.getSection("dict_gen_name_string_cnt_file");
		String midEachAliasCntFileName = sect.getValue("each_alias_cnt_file"),
				dstFileName = sect.getValue("dst_file");
		DictGen.genNameStringCntFile(midEachAliasCntFileName, dstFileName);
	}

	public static void test() {
		DataInputStream dis = IOUtils.getBufferedDataInputStream("d:/data/el/name_string_cnt.bin");
		
		try {
			System.out.println("loading");
			int numNames = dis.readInt();
			ByteArrayString[] names = new ByteArrayString[numNames];
			int[] cnts = new int[numNames];
			for (int i = 0; i < numNames; ++i) {
				names[i] = new ByteArrayString();
				names[i].fromFileWithShortLen(dis);
				cnts[i] = dis.readInt();
			}
			dis.close();
			System.out.println("done");
			
			String sname = "Michael";
			ByteArrayString bas = new ByteArrayString(sname);
			int pos = Arrays.binarySearch(names, bas);
			System.out.println(cnts[pos]);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void run(IniFile config) {
		...
		else if (job.equals("dict_gen_name_string_cnt_file"))
			genNameStringCntFile(config);
	}
}
