...
public class QueryReader {
	...
	public static Document[] toDocuments(String queryFileName, boolean withPosition) {
		BufferedReader reader = IOUtils.getUTF8BufReader(queryFileName);

		LinkedList<Query> queries = new LinkedList<Query>();
		Query query = null;
		try {
			while ((query = nextQuery(reader, withPosition)) != null) {
				queries.add(query);
			}
	...
	private static Query nextQuery(BufferedReader bufReader, boolean withPosition) {
		String line = null;
	...
	public Query nextQuery(boolean withPosition) {
		return nextQuery(reader, withPosition);
	}
